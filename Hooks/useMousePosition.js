/*
  The useMousePosition() hook lets you track the mouse position when 
  hovering over a specific container or the entire page, 
  so if a target container is not provided through the ref,
...
export function useMousePosition() {
  ...
  const setMousePosition = (event) => {
    if (target.current) {
      ...
      const y = max(
        0,
        Math.round(event.pageY - r.top - (window.pageYOffset || window.scrollY))
      );
      setPosition({ x, y });
